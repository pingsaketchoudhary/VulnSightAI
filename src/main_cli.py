...
from report_generator import generate_html_report, save_pdf_report # Naya import

def main():
    """
    Tool ke liye main function, jo user input ko handle karta hai.
    """
    parser = argparse.ArgumentParser(description="VulnSightAI - Ek AI-Powered Reconnaissance Toolkit")
    
    parser.add_argument("-t", "--target", required=True, help="Target domain jise scan karna hai (e.g., example.com)")
    parser.add_argument("-oJ", "--output-json", help="Scan ke result ko save karne ke liye JSON file ka naam (e.g., result.json)")
    
    # Naye arguments report ke liye
    parser.add_argument("--html", help="HTML report generate karein aur is file path par save karein (e.g., report.html)")
    parser.add_argument("--pdf", help="PDF report generate karein aur is file path par save karein (e.g., report.pdf)")
    ...
    # HTML aur PDF report generate karne ka logic
